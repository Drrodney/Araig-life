			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) == localize "str_channel_side") then 
			{
				...
				if (disconnect_me == 0) then {
					if (isNil "warn_one") then {
						warn_one = true;
						systemChat ("Please DO NOT use the sidechat channel! This is your first warning.");
						[] spawn DS_slap_them;
						player setDamage 0.20;
						//["beat04"] spawn DS_really_loud_sounds;
						["FORBIDDEN VOICE IN SIDE!"] spawn DS_double_cut;
					};
				};
				if (disconnect_me == 1) then {
					if (isNil "warn_two") then {
						warn_two = true;
						systemChat ("Please DO NOT use the sidechat channel! This is your second warning...");
						[] spawn DS_slap_them;
				...
				if (disconnect_me == 2) then {
					if (isNil "warn_three") then {
						warn_three = true;
						systemChat ("Please DO NOT use the sidechat channel! This is your last warning...");
						systemChat ("You will be frozen and then disconnected!");
						player setDamage 0.80;
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["FORBIDDEN VOICE IN SIDE!! This is your last warning!"] spawn DS_double_cut;
					};
				};
				if (disconnect_me >= 3) then {
					if (isNil "warn_last") then {
						...
						startLoadingScreen ["You are being kicked for using side chat."];
						...
						endMission "THANK YOU - Please call again";
					};
				};
			};
		};
